...
class PartialRenderer extends ReactDOMServerRenderer {
	...
	handleNoSsrPromise(promise) {
		...
		// Find enclosing suspense boundary
		let boundary = this.node;
		while (boundary && boundary.type !== TYPE_SUSPENSE) {
			boundary = boundary.parent;
			this.nonDomFrames--;
		}

		// If no enclosing suspense boundary, exit with error
		if (!boundary) return this.halt(promise);

		// Abort all promises within boundary
		this.abortDescendents(boundary);

		// Convert boundary node to fallback node (+ remove all children)
		const {frame: boundaryFrame, fallback, stackState} = boundary;
		boundary.type = null;
		boundary.children.length = 0;
		boundary.fallback = null;
		boundary.stackState = null;
