...
const PartialRenderer = require('./renderer'),
	{TYPE_SUSPENSE, TYPE_PROMISE, NO_SSR} = require('./constants'),
 ...
 * @returns {undefined}
 */
function abortDescendents(node) {
	for (let child of node.children) {
		// Abort if promise not resolved already
		if (child.type === TYPE_PROMISE && !child.resolved) {
			child.aborted = true;

			// Call `.abort()` method on promise
			const {promise} = child;
			if (typeof promise.abort === 'function') promise.abort();
		}

		abortDescendents(child);
	}
}
