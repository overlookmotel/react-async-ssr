...
class AsyncRenderer {
	...
	*/
	...
	resolveLazyNodes(lazyNodes, cb) {
		async.map(lazyNodes, (node, cb) => this.resolveLazy(node, cb), cb);
	}
	...
	resolveLazy(node, cb) {
		...
		const {promise} = node;
		if (promise[NO_SSR]) return this.resolveFailedLazy(node, cb);
		...
		return promise.then(
			() => {
				...
				if (node.aborted) return cb();
				node.resolved = true;

				this.renderChild(node, cb);
			},
			err => {
	...
	resolveFailedLazy(node, cb) {
		...
		if (!boundary) return cb(node.promise);
