...
class AsyncRenderer {
	...
	resolveFailedLazy(node) {
		...
		// Register handler on promise to prevent unhandled rejection
		node.promise.then(() => {}, () => {});

		// Make inactive all boundary's children and descendents.
		// i.e. prevent them continuing rendering.
		makeDescendentsInactive(boundary);

		// Empty contents of boundary
		boundary.children = [];
		boundary.footer = null;
...
/**
 * Make all descendents (children, children's children etc) of a node inactive.
 * They will then abandon rendering and any errors thrown will be silently
 * swallowed.
 * @param {Object} node - Starting node
 * @returns {undefined}
 */
function makeDescendentsInactive(node) {
	for (let child of node.children) {
		child.inactive = true;
		makeDescendentsInactive(child);
	}
}
